...
func main() {
	...
	mgr, err := ctrl.NewManager(ratelimiter.LimitRESTConfig(cfg, *maxReconcileRate), ctrl.Options{
		...
		LeaderElectionID: "crossplane-leader-election-provider-upjet-gcp-beta-dataform",
		Cache: cache.Options{
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(controllerCluster.SetupGated_dataform(mgr, optCluster), "Cannot setup GCP controllers")
		kingpin.FatalIfError(controllerNamespaced.SetupGated_dataform(mgr, optNamespaced), "Cannot setup GCP controllers")
	} else {
		logr.Info("Provider has missing RBAC permissions for watching CRDs, controller SafeStart capability will be disabled")
		kingpin.FatalIfError(controllerCluster.Setup_dataform(mgr, optCluster), "Cannot setup GCP controllers")
		kingpin.FatalIfError(controllerNamespaced.Setup_dataform(mgr, optNamespaced), "Cannot setup GCP controllers")
	}
