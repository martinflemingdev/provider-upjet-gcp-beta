...
import (
	...
	apisCluster "github.com/upbound/provider-gcp-beta/apis/cluster"
	apisNamespaced "github.com/upbound/provider-gcp-beta/apis/namespaced"
	...
	controllerCluster "github.com/upbound/provider-gcp-beta/internal/controller/cluster"
	controllerNamespaced "github.com/upbound/provider-gcp-beta/internal/controller/namespaced"
	"github.com/upbound/provider-gcp-beta/internal/features"
	"github.com/upbound/provider-gcp-beta/internal/version"
)
...
func main() {
	...
	logr := logging.NewLogrLogger(zl.WithName("provider-upjet-gcp-beta"))
	if *debug {
	...
	mgr, err := ctrl.NewManager(ratelimiter.LimitRESTConfig(cfg, *maxReconcileRate), ctrl.Options{
		...
		LeaderElectionID: "crossplane-leader-election-provider-upjet-gcp-beta-vertexai",
		Cache: cache.Options{
	...
	pc, err := config.GetProvider(ctx, sdkProvider, false)
	kingpin.FatalIfError(err, "Cannot initialize the provider configuration")
	pns, err := config.GetProviderNamespaced(ctx, sdkProvider, false)
	kingpin.FatalIfError(err, "Cannot initialize the provider configuration")
	optCluster := tjcontroller.Options{
		...
		Provider:              pc,
		SetupFn:               clients.TerraformSetupBuilder(pc.TerraformProvider),
		PollJitter:            pollJitter,
		OperationTrackerStore: tjcontroller.NewOperationStore(logr),
		StartWebhooks:         *certsDir != "",
	}
	optNamespaced := tjcontroller.Options{
		Options: xpcontroller.Options{
			Logger:                  logr,
			GlobalRateLimiter:       ratelimiter.NewGlobal(*maxReconcileRate),
			PollInterval:            *pollInterval,
		...
		Provider:              pns,
		SetupFn:               clients.TerraformSetupBuilder(pns.TerraformProvider),
		PollJitter:            pollJitter,
		OperationTrackerStore: tjcontroller.NewOperationStore(logr),
		StartWebhooks:         *certsDir != "",
	}
	...
	if *enableManagementPolicies {
		optCluster.Features.Enable(features.EnableBetaManagementPolicies)
		optNamespaced.Features.Enable(features.EnableBetaManagementPolicies)
		logr.Info("Beta feature enabled", "flag", features.EnableBetaManagementPolicies)
	}

	if *enableChangeLogs {
		optCluster.Features.Enable(feature.EnableAlphaChangeLogs)
		optNamespaced.Features.Enable(feature.EnableAlphaChangeLogs)
		...
		optCluster.ChangeLogOptions = &clo
		optNamespaced.ChangeLogOptions = &clo
	}
	...
	if canSafeStart {
		...
		optCluster.Gate = crdGate
		optNamespaced.Gate = crdGate
		kingpin.FatalIfError(customresourcesgate.Setup(mgr, optNamespaced.Options), "Cannot setup CRD gate")
		kingpin.FatalIfError(controllerCluster.SetupGated_vertexai(mgr, optCluster), "Cannot setup GCP controllers")
		kingpin.FatalIfError(controllerNamespaced.SetupGated_vertexai(mgr, optNamespaced), "Cannot setup GCP controllers")
	} else {
		logr.Info("Provider has missing RBAC permissions for watching CRDs, controller SafeStart capability will be disabled")
		kingpin.FatalIfError(controllerCluster.Setup_vertexai(mgr, optCluster), "Cannot setup GCP controllers")
		kingpin.FatalIfError(controllerNamespaced.Setup_vertexai(mgr, optNamespaced), "Cannot setup GCP controllers")
	}

	kingpin.FatalIfError(conversion.RegisterConversions(optCluster.Provider, optNamespaced.Provider, mgr.GetScheme()), "Cannot initialize the webhook conversion registry")
